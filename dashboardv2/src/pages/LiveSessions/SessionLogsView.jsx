...
export default function SessionLogsView() {
  ...
  // Resolved entity states for direct ID deep-linking
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then(([stationsRes, chargePointsRes]) => {
        ...
        // 1. Resolve Charging Station by ID or Name match
        ...
        // 2. Resolve Charge Point by ID or Name match
  ...
  // Direct Navigation to View Charging Station page
  ...
  // Direct Navigation to View Charge Point page
    ...
    <div className="flex flex-col gap-4 max-w-[1400px] w-full mx-auto pb-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              >
                ...
                <span>{sessionData.status === 'Ongoing' ? 'LIVE' : sessionData.status}</span>
              </span>
            </div>

            {/* Sub-line with Clickable Station and Charge Point links */}
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {/* Interactive Copy Session ID Capsule */}
          <button
            onClick={handleCopySessionId}
            className={`group relative flex items-center gap-2 px-3 py-1.5 rounded-xl text-xs font-extrabold border transition-colors duration-150 cursor-pointer active:scale-95 shadow-2xs ${
              copiedSessionId
          ...
          >
            ...
            <span className="font-mono font-extrabold tracking-tight">{sessionData.id}</span>

            {copiedSessionId ? (
              <Check className="w-3.5 h-3.5 text-emerald-600 stroke-[2.5] animate-in zoom-in-75 duration-150 ml-0.5" />
            ) : (
              <Copy className="w-3.5 h-3.5 text-stone-400 group-hover:text-violet-600 group-hover:scale-105 transition-all duration-150 ml-0.5" />
            )}
          </button>

          {/* Interactive Refresh / Sync Log Stream Button */}
          <button
            onClick={handleManualRefresh}
            className={`group flex items-center gap-2 px-3 py-1.5 rounded-xl text-xs font-extrabold border transition-colors duration-150 cursor-pointer active:scale-95 shadow-2xs ${
              isRefreshing
      ...
      <ChargePointLogsTab cp={cpMock} sessionData={sessionData} />
