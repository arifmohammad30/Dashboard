...
export default function DashboardLayout({ children }) {
  ...
  return (
    <div className="flex h-screen w-full overflow-hidden text-stone-800 relative z-0 bg-orange-50/50">
      {/* Sunset Animated Mesh Gradient Background (GPU Accelerated) */}
      <div className="fixed inset-0 overflow-hidden pointer-events-none -z-10 transform-gpu">
        <div className="absolute top-[-10%] left-[-10%] w-[50%] h-[50%] rounded-full bg-orange-500/20 blur-[80px] will-change-transform transform-gpu" />
        <div className="absolute bottom-[-10%] right-[-10%] w-[50%] h-[50%] rounded-full bg-rose-500/15 blur-[80px] will-change-transform transform-gpu" />
        <div className="absolute top-[30%] right-[20%] w-[40%] h-[40%] rounded-full bg-amber-400/20 blur-[70px] will-change-transform transform-gpu" />
        <div className="absolute bottom-[20%] left-[20%] w-[35%] h-[35%] rounded-full bg-yellow-400/15 blur-[70px] will-change-transform transform-gpu" />
      </div>
      ...
      <div className="flex-1 flex flex-col h-full overflow-hidden w-full relative z-0">
        <header className="h-16 flex items-center justify-between px-6 bg-white/80 border border-white/70 shadow-xs shrink-0 z-10 mx-4 md:mx-8 mt-4 rounded-2xl">
          ...
          <div className="flex items-center gap-4">
            <button className="p-2.5 rounded-xl bg-stone-100/80 hover:bg-stone-200/80 transition cursor-pointer outline-none flex items-center justify-center">
              <svg className="w-5 h-5 text-amber-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9"></path></svg>
            </button>

            <div className="relative" ref={userMenuRef}>
              <div
                className="w-10 h-10 rounded-full bg-stone-100/80 hover:bg-stone-200/80 border border-stone-200/60 flex items-center justify-center cursor-pointer transition-colors duration-200"
