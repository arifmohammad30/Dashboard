  ...
  return (
    <div className="relative inline-block" ref={popoverRef}>
      <button
        ...
        className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-[11px] font-semibold bg-cyan-50/90 hover:bg-cyan-100/90 text-cyan-800 border border-cyan-200/80 cursor-pointer shadow-2xs transition-all duration-150"
        title="Click to view State of Charge details"
      >
        <Battery className="w-3 h-3 text-cyan-600 shrink-0" />
        <span className="font-mono font-bold text-cyan-900">{currentDisplay}</span>
        <ChevronDown className={`w-3 h-3 text-cyan-500 transition-transform duration-150 ${isOpen ? 'rotate-180 text-cyan-800' : ''}`} />
      </button>
      ...
      {isOpen && (
        <div className="absolute left-0 top-full mt-1.5 z-40 bg-white border border-stone-200 shadow-xl rounded-xl p-2.5 min-w-[165px] text-xs animate-in fade-in zoom-in-95 duration-150 font-sans">
          ...
          <div className="flex flex-col gap-1.5 text-[11px]">
            ...
            <div className="flex justify-between"><span className="text-stone-500">Current SoC:</span> <strong className="font-mono text-cyan-700">{currentDisplay}</strong></div>
          </div>
        </div>
      )}
    </div>
